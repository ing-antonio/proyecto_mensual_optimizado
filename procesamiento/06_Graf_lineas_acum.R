...
generar_grafico_delito <- function(nombre_delito, datos) {
  ...
  resumen <- datos_filtrados %>%
    ...
    mutate(
      dias_acumulados = map_dbl(year, ~{
        ...
        fecha_fin_anio <- ceiling_date(as.Date(paste0(.x, "-", ultimo_mes_completo6, "-01")), "month") - days(1)
  ...
  if (nrow(resumen) == 0) {
    message(glue("No hay datos para {nombre_delito}, se omite la gráfica"))
    return(NULL)
  }
  ...
  p <- ggplot(resumen, aes(x = factor(year), y = promedio_diario)) +
    ...
    if(nombre_delito == "Homicidio doloso"){
      scale_y_continuous(labels = scales::comma_format(accuracy = 0.1), limits = c(min(resumen$promedio_diario), max(resumen$promedio_diario)))
    }
  
    if(nombre_delito == "Homicidio doloso (víctimas)"){
      scale_y_continuous(labels = scales::comma_format(accuracy = 0.1), limits = c(min(resumen$promedio_diario), max(resumen$promedio_diario)))
    }
    
    if(nombre_delito == "Lesiones dolosas por disparo de arma de fuego"){
      scale_y_continuous(labels = scales::comma_format(accuracy = 0.1), limits = c(min(resumen$promedio_diario), max(resumen$promedio_diario)))
    }
  
    if(nombre_delito == "Robo de vehículo con y sin violencia"){
      scale_y_continuous(labels = scales::comma_format(accuracy = 0.1), limits = c(min(resumen$promedio_diario) - 1, max(resumen$promedio_diario) + 1))
    }
...
# EL ARGUMENTO DATOS = CONSULTA_TOTAL6 SE OBTUVO DEL QUERY Y SE PASA PARA TODAS LAS EJECUCIONES
walk(delitos_deseados, generar_grafico_delito, datos = consulta_total6)
